...
//***********************************************************
// Nasa is with 2 independent USB port
...
{
    #if ENABLE_SC_BLOCK_SYNC_WAKEUP
    //[Description]: This fixes the issue that it sometimes cannot get Vtotal when DPM DC is turned off
    //[Solution] : Clock id2clk needs to be turned on to fix this issue.
    //[Side-effect] This will increase the power consumption.
    MDrv_Write2Byte(0x100baa, 0); // reg_ckg_idclk2
    #endif

    MDrv_Write2Byte(REG_PM_SD_POL, MDrv_Read2Byte(REG_PM_SD_POL)|(BIT6));  //1: active low
    //[Description]: This fixes the issue that entering SYNC_ISR only once with some special test patterns.
    //[Solution] : Using unmasking hsync0 and vsync0 
    MDrv_WriteByte(REG_PM_SD_IRQ_MASK, MDrv_ReadByte(REG_PM_SD_IRQ_MASK)&(~(BIT0))); //unmask hsync0
...
#if 1//Check only one DVI port which triggers DPMS standby
...
#else //check all DVI ports
void MDrv_DVI_ManualSwitchPort(void)
{
    #define DELAY_TIME_MS   20 //>=500 :OK, <500:with risk of failure

    if(MDrv_ReadByte(REG_PM_GCR_SWCK)&BIT3)
    {
        //switch to port 1
        MDrv_WriteRegBit(REG_PM_GCR_SWCK, 0, BIT3);
        msDelayTime(DELAY_TIME_MS);
        MDrv_WriteRegBit(REG_PM_GCR_SWCK, 1, BIT4);
    }
    else if(MDrv_ReadByte(REG_PM_GCR_SWCK)&BIT4)
    {
        //switch to port 2
        MDrv_WriteRegBit(REG_PM_GCR_SWCK, 0, BIT4);
        msDelayTime(DELAY_TIME_MS);
