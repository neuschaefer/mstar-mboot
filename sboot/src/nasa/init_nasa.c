...
#define NASA_MIU_UNIT 0x8
#define NASA_FPGA_VERIFY 0
...
static void MDrv_SysInitMPLL( void )
{
    ...
    // DSP PLL Setting
    MDrv_WriteByte(0x110c40, 0x54);
    MDrv_WriteByte(0x110c41, 0x00);
    //MIPSPLL_TEST             0x110C40[15:0]  = 16'd0 (default) => 16'h40

    MDrv_WriteByte(0x110c44, 0x20);
    MDrv_WriteByte(0x110c45, 0x00);
    ...
    MDrv_WriteByte(0x110c46, 0x32);
    MDrv_WriteByte(0x110c47, 0x00);
    ...
    //MIPSPLL_LOOP_DIV_SECOND  0x110C46[7:0]   = 8'h16 (default) => 8'h2C
  	...
  	MDrv_WriteByte(0x110c42, 0x00);
  	MDrv_WriteByte(0x110c43, 0x00);
...
///#################################
static void MDrv_USBPLL_Port0_Init( void )
{
    // turn 480Mhz in USB
    // USBC0 <--> UTMI0
    ...
    MDrv_WriteByte(0x103aa2, 0x88);    // Set PLL_TEST[23:22] for enable 480MHz clock
    MDrv_WriteByte(0x103aa0, 0x51);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    #if (RTL_SIMULATION == 1)
    MDrv_SysDelayUs(1);
    #else
    ...
    MDrv_WriteByte(0x103aa3, 0x20);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    MDrv_WriteByte(0x103aa4, 0x00);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    #if (RTL_SIMULATION == 1)
    MDrv_SysDelayUs(1);
    #else
    MDrv_SysDelayUs(1000);             // delay 1ms
    ...
    MDrv_WriteByte(0x103a89, 0x04);    // #3=0 Disable force_pll_on
    MDrv_WriteByte(0x103a88, 0x0f);    // #7=0 Enable band-gap current
...
static void MDrv_USBPLL_Port1_Init( void )
{
    // turn 480Mhz in USB
    // USBC0 <--> UTMI0
    ...
    MDrv_WriteByte(0x103a22, 0x88);    // Set PLL_TEST[23:22] for enable 480MHz clock
    MDrv_WriteByte(0x103a20, 0x51);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    #if (RTL_SIMULATION == 1)
    MDrv_SysDelayUs(1);
    #else
    MDrv_SysDelayUs(1000);             // delay 1ms
    #endif
    MDrv_WriteByte(0x103a21, 0x80);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    MDrv_WriteByte(0x103a23, 0x20);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    MDrv_WriteByte(0x103a24, 0x00);    // UTMI offset 0x20 bit1=1//Set PLL_TEST[1] for PLL multiplier 20X
    #if (RTL_SIMULATION == 1)
    MDrv_SysDelayUs(1);
    #else
    MDrv_SysDelayUs(1000);             // delay 1ms
    #endif
    MDrv_WriteByte(0x103a02, 0x84);    // Enable CLK12_SEL bit <2> for select low voltage crystal clock
    MDrv_WriteByte(0x103a03, 0x20);    // Enable CLK12_SEL bit <2> for select low voltage crystal clock
    MDrv_WriteByte(0x103a09, 0x04);    // #3=0 Disable force_pll_on
    MDrv_WriteByte(0x103a08, 0x0f);    // #7=0 Enable band-gap current
    MDrv_WriteByte(0x103a01, 0x6b);    // Turn on reference voltage and regulator
    MDrv_WriteByte(0x103a00, 0xc3);    // reg_pdn: bit<15>, bit <2> ref_pdn
    MDrv_SysDelayUs(1000);             // delay 1ms
    MDrv_WriteByte(0x103a01, 0x69);    // Turn on UPLL, reg_pdn: bit<9>
    MDrv_SysDelayUs(2000);             // delay 2ms
    MDrv_WriteByte(0x103a00, 0x01);    // Turn all (including hs_current) use override mode
    MDrv_WriteByte(0x103a01, 0x00);
}

static void MDrv_USBPLL_Init( void )
{
    MDrv_USBPLL_Port0_Init();
    MDrv_USBPLL_Port1_Init();
}
...
{
    _RV32_1(0x100b01, 0x10),  // turn xtal clk  #4 = 1 (CKg_RIU must be "1" in VIF input)
    ...
    // dig_top clk_spi
    _RV32_1(0x100b2c, 0x14),      // clk spi to 108Mhz
    ...
    // clk_mail
    _RV32_1(0x100b44, 0x00),      //clk mail box0 =xtal  <<hk51 <--mail box 0--> aeon
    _RV32_1(0x100b45, 0x00),      //clk mail box1 =xtal  <<hk51 <--mail box 1--> aeon
    ...
    _RV32_1(0x100b66, 0x00),      //ckg_parser =172Mhz
    _RV32_1(0x100b6a, 0x0c),      //ckg_jpg=160M  0:72m  4:123m  8:144m  c:160m
    // clk_mvdxx
...
static int InitialSystemPowerUp( void )
{
    ...
    //
    MDrv_WriteByte(0x000ae2, 0xFF);
    MDrv_WriteByte(0x000ae3, 0x0F);
    MDrv_WriteByte(0x000E6A, 0x30);      // max SPI pad driving
    MDrv_WriteByte(0x000ae4, 0x01);      //Enable fast read mode, (SPI command is 0x0B)
...
static void MDrv_MiuReset( void )
{
    ...
    MDrv_Write2Byte( 0x110d00, 0x0010 );
    MDrv_Write2Byte( 0x110d00, 0x0000 );
    MDrv_Write2Byte( 0x110d7c, 0x0002 );
    MDrv_Write2Byte( 0x110d02, 0xaaaa );
    if(MDrv_ReadRegBit(0x101ec1,BIT2))//G_1066
    {
        MDrv_Write2Byte( 0x110d74, 0x0022 ); //trig lvl
        MDrv_Write2Byte( 0x110d78, 0x0002 );
        MDrv_Write2Byte( 0x110d5c, 0x5555 ); //drvn
        MDrv_Write2Byte( 0x110d5e, 0x5555 ); //drvp
    }
    else//G_1033
    {
        MDrv_Write2Byte( 0x110d74, 0x0033 ); //trig lvl
        MDrv_Write2Byte( 0x110d5c, 0x3333 ); //drvn
        MDrv_Write2Byte( 0x110d5e, 0x3333 ); //drvp
    }
  #endif
...
static void MDrv_Nasa_BandWidth_Adjustment(void)
{
    //-------------
    // BW setting
    //-------------
  ...
  #endif
    MDrv_Write2Byte(0x101280,0x8015); //enable group 2 run-robin
    MDrv_Write2Byte(0x1012a0,0x8015);

  #if 0//default bandwidth settings
    //Let these IPs HVD/MVD/G3D(removed in Nasa) use MIU Act 2 bank function to improve DRAM total Bandwidth utilization
...
static void MDrv_MiuAtopProgramDLL0(void)
{
    ...
    MDrv_Write2Byte(0x110d60, 0x2370);
    //-------------
    //Wait CAL Done
    //-------------
    MDrv_SysDelayUs(1*1000);  // delay 1ms
    MDrv_Write2Byte(0x110d08, 0x00ff);
    MDrv_Write2Byte(0x110d0e, 0x00a9);
    MDrv_Write2Byte(0x110d0e, 0x00e9);
    MDrv_Write2Byte(0x110d0e, 0x00a9);
...
static void MDrv_MiuBistTest( void )
{
    ...
    MDrv_Write4Byte((RIUBASE_MIU + (0x0072 *2)),(MIU0_SIZE / NASA_MIU_UNIT )  ); //test length
    MDrv_Write2Byte((RIUBASE_MIU + (0x0074 *2)), 0x5aa5);   //test data
...
static int MDrv_Nasa_MiuInitial(void)
{
    #if NASA_FPGA_VERIFY
    {
    ...
    //Nasa MIU atop initial
    MDrv_Write2Byte(0x101220, 0x0000);
    MDrv_Write2Byte(0x100620, 0x0000);
    MDrv_Write2Byte(0x10121e, 0x0c00);
    MDrv_Write2Byte(0x10121e, 0x0c00);
    MDrv_Write2Byte(0x10121e, 0x0c00);
    MDrv_Write2Byte(0x10121e, 0x0c01);
    ...
    if(MDrv_ReadRegBit(0x101ec1,BIT2))//G_1066
    {
        MDrv_WriteMiuAtopTbl (  MIU_ATOP_Init_1066 );
    }
    else//G_1333
    {
        MDrv_WriteMiuAtopTbl (  MIU_ATOP_Init_1333 );
    }
  #endif
    ...
    if(MDrv_ReadRegBit(0x101ec1,BIT2))//G_1066
    {
        ...
        {
            MDrv_WriteMiuTbl (  MIU_DDR_Init_1066_64m );
        }
        else//MCP 32M
        {
            MDrv_WriteMiuTbl (  MIU_DDR_Init_1066_32m );
        }
    }
    else//G_1333
    {
        MDrv_WriteMiuTbl (  MIU_DDR_Init_1333 );
    }
  #endif

  #if 0//(ENABLE_MSTAR_NASA_MMAP_32MB)
    ...
    MDrv_Nasa_BandWidth_Adjustment();
    //MDrv_MiuAtopProgramDLL0();

    //set dram size //0x1012_60[11:8]
  #if 0//(ENABLE_MSTAR_NASA_MMAP_32MB)
    MDrv_Write2Byte(0x1012c0, 0x0500 ); //reg_dram_size = 32MB
  #else
    if(MDrv_ReadRegBit(0x101ec1,BIT3))//MCP 64M
    {
    ...
    {
        MDrv_Write2Byte(0x1012c0, 0x0500 );
    }
  #endif
...
void BL_Init(void)
{
    ...
    // init piu timer1 for auto test
    MDrv_Write2Byte(0x3044, 0xFFFF);
    MDrv_Write2Byte(0x3046, 0xFFFF);
    MDrv_Write2Byte(0x3040, 0);
    MDrv_Write2Byte(0x3040, 1);
  ...
  #if ((MS_BOARD_TYPE_SEL == BD_XL3_MEGA_NASA))
  {
    ...
    //For B-power on, pad init should be executed befor MIU init
    //(3)Pad Init 
    MDrv_Pad_Init();
    MDrv_SysDelayUs(10000);   //delay 10ms
  }
  #endif
    ...
    // update CBUS FT test pattern for CBUS pull low resistor
    if(1)
    {
        U8 u8Val;
        //check if update bit0 ~ bit3
        if(MDrv_ReadRegBit(0x2030,BIT4))
        {
            // read from efuse table
            u8Val = MDrv_ReadByte(0x2030);
            // update settings for CBUS PD_1K_LOW/HIGH, CBUS PD_100K_LOW/HIGH
            MDrv_WriteByteMask(0x101e1c, (U8)(u8Val<<3), BIT6|BIT5|BIT4|BIT3);
        }
    }

    // MIU initialization
    MDrv_Nasa_MiuInitial();
