...
#define EDEN_MIU_UNIT 0x8
#define EDEN_FPGA_VERIFY 0
...
static void MDrv_Eden_BandWidth_Adjustment(void)
{
    ...
    //Let these IPs HVD/MVD/G3D(removed in Eden) use MIU Act 2 bank function to improve DRAM total Bandwidth utilization
...
static void MDrv_MiuBistTest( void )
{
    ...
    MDrv_Write4Byte((RIUBASE_MIU + (0x0072 *2)),(MIU0_SIZE / EDEN_MIU_UNIT )  ); //test length
    MDrv_Write2Byte((RIUBASE_MIU + (0x0074 *2)), 0x5aa5);   //test data
...
static int MDrv_Eden_MiuInitial(void)
{
    #if EDEN_FPGA_VERIFY
    {
  ...
  #elif (MIU_SPEED == DDR2_1066MHz)//MCLK 533MHz setting
    MDrv_WriteMiuTbl (  MIU_DDR_Init_1066 );
  #else
    #if 1
    ...
    MDrv_Eden_BandWidth_Adjustment();
    MDrv_MiuAtopProgramDLL0();

    //set dram size //0x1012_60[11:8]
  #if (ENABLE_MSTAR_EDEN_MMAP_32MB)
    MDrv_Write2Byte(0x1012c0, 0x0600 ); //this value must be set >= 6
  #else
    if(MDrv_ReadRegBit(0x101ec1,BIT3))//MCP 64M
    {
...
void BL_Init(void)
{
  ...
  #if(MS_BOARD_TYPE_SEL==BD_MST202A_D01A_S)
  {
    ...
    if((MDrv_ReadByte(0x101ecf)>=(U8)0x02)||(MDrv_ReadByte(0x101ecc)==(U8)0x5E))
    {
        U8 u8Val;
        //check if update bit1 & bit0
        if(MDrv_ReadRegBit(0x200e,BIT2))
        {
            // read from efuse table
            u8Val = MDrv_ReadByte(0x200e);
            // update settings
            MDrv_WriteByteMask(0x101e3c, u8Val, BIT1|BIT0);
        }
    }

    // MIU initialization
    MDrv_Eden_MiuInitial();
