...
#define M10_MIU_UNIT 0x8
#define M10_GPGA_VERIFY 0
...
//   U8 u8Length;
   ...
   //RIU[0x3c60]=pRegTable[u16Index];
   //RIU[0x3c72]=0xaa55;
   //MDrv_Write2Byte(0x3008, 0x0000);//Disable watch dog
   //MDrv_Write2Byte(0x300A, 0x0000);//Disable watch dog
       //printf("2nd: %x %x %x\n", (U32)loader_2nd_vma, (U32)loader_2nd_lma, (U32)loader_2nd_len);
       // Load 2nd loader to DRAM
   //CopyFLH2MIU( (U32) loader_2nd_vma, (U32)loader_2nd_lma, (U32) loader_2nd_len);

       //risc32_regbase[0xF0] = 0x43;    // SPI boot, Enable Aeon
       //RIU[0x101E00] |= BIT1;    // over_write_chip_config
 ...
 /*  __asm__ __volatile__ (
       ...
       "\tl.sw     0(%0),r7\n"
       "\tl.jr     %1\n"
       : : "r" (&risc32_regbase[0xF0] ), "r" (loadAP) : "r7");
*/
 /*   do
    {
...
static void  MDrv_AutoInitMiuCycle(void)
{
    ...
    MDrv_Write2Byte((RIUBASE_MIU ), 0x001E);    //reg_0x1200= 0x000E,DRAmreset+DRAM chip select+enable CKE + ODT ON
  #elif (MIU_SPEED == DDR_400Mhz_2x)
    MDrv_Write2Byte((RIUBASE_MIU ), 0x000E);    //reg_0x1200= 0x000E,DRAmreset+DRAM chip select+enable CKE
  #else
    ...
    MDrv_SysDelayUs(1);
  #else
    MDrv_SysDelayUs(400); // delay 400us
  #endif


  #if (MIU_SPEED == DDR2_800MHz)
    MDrv_Write2Byte((RIUBASE_MIU ), 0x001F);    //reg_0x1200= 0x000F,DRAmreset+DRAM chip select+enable CKE +auto initial dram cycle + ODT ON
    #if(ENABLE_MSTAR_MARIA10_BD_MST136A_D01A_S)
    MDrv_Write2Byte((RIUBASE_MIU ), 0x000F);
    #endif
  #elif (MIU_SPEED == DDR_400Mhz_2x)
...
static void MDrv_MiuReset( void )
{
    ...
    #elif (MIU_SPEED == DDR_400Mhz_2x)
    MDrv_Write2Byte( 0x110d00, 0x0100 );  // 0x0000 ->0x0100
    #else
    #error "MIU interface isn't defined"
    #endif
}
...
#if 0
static void MDrv_Maria10_BandWidth_Adjustment(void)
{
    MDrv_Write2Byte(0x101240,0xC000);
    MDrv_Write2Byte(0x101260,0xE000);
    MDrv_Write2Byte(0x101280,0xC005); //enable group 2 run-robin
    MDrv_Write2Byte(0x101282,0x0018);
    MDrv_Write2Byte(0x101268,MDrv_Read2Byte(0x101268) & 0xFC17);
    MDrv_Write2Byte(0x101288,MDrv_Read2Byte(0x101288) & 0xFFF7);
    MDrv_WriteByte(0x1012DC,0xE1);
    MDrv_WriteByteMask(0x1012DD,0x01,0x01);
    MDrv_WriteByteMask(0x1012DD,0x00,0x01);
}
#endif
...
static void MDrv_MiuBistTest( void )
{
    ...
    MDrv_Write4Byte((RIUBASE_MIU + (0x0072 *2)),(MIU0_SIZE / M10_MIU_UNIT )  ); //test length
    MDrv_Write2Byte((RIUBASE_MIU + (0x0074 *2)), 0x5aa5);   //test data
...
static int MDrv_Maria10_MiuInitial(void)
{
    #if M10_GPGA_VERIFY
    {
    ...
    //Maria10 MIU atop initial
    MDrv_Write2Byte((0x10121e ) , 0x0c00);
    MDrv_Write2Byte((0x10121e ) , 0x0c01);
    MDrv_Write2Byte((0x10121e ) , 0x0c00);
    MDrv_Write2Byte((0x101202 ) , 0x0000);

    MDrv_Write2Byte((0x1012f8 ) , 0x8000);
    ...
    MDrv_Write2Byte((0x1012c0 ) , 0x0000);  //un_protect all MIU
    MDrv_Write2Byte((0x10121e ) , 0x0000);
    MDrv_Write2Byte((0x101200 ) , 0x0000);

    MDrv_WriteMiuTbl ( MIU_DDR_Init );      //initial miu_0 /MIU_1
    MDrv_MiuReset();
    MDrv_Write2Byte(0x110d08 , 0xffff);
    ...
    #if 0
    MDrv_Maria10_BandWidth_Adjustment();
    #endif

    MDrv_Write2Byte(0x101228, (MDrv_Read2Byte(0x101228) | BIT10));  // i32 mode
    MDrv_Write2Byte(0x101206, (MDrv_Read2Byte(0x101206) | BIT11));  // Compression mode en
