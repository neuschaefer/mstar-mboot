...
void MDrv_DVI_ConfigWakeup(void)
{
    ...
    MDrv_Write2Byte(REG_PM_GCR_SWCK, 0x00F0);
    ...
    MDrv_Write2Byte(REG_PM_CMPVALUE2_P0, 0x0014);// reg_cmpvalue2 //for sleep
    ...
    MDrv_Write2Byte(REG_PM_CMPVALUE2_P1, 0x0014);// reg_cmpvalue2_1 //for sleep
...
void MDrv_DVI_ManualSwitchPort(void)
{
    ...
    {
        MDrv_WriteByte(REG_PM_GCR_SWCK, MDrv_ReadByte(REG_PM_GCR_SWCK)&(~BIT3));
        MDrv_WriteByte(REG_PM_GCR_SWCK, MDrv_ReadByte(REG_PM_GCR_SWCK)|(BIT4));
    }
    else
    {
        MDrv_WriteByte(REG_PM_GCR_SWCK, MDrv_ReadByte(REG_PM_GCR_SWCK)&(~BIT4));
...
void MDrv_CheckWakeupDeviceFromAP(void)
{
    ...
    {
        MDrv_WriteRegBit(0x10250b, 0, _BIT0); //adc_atop::reg_pdn_hsync[0], 1 : power down, 0 : power up
        msDelayTime(10);
