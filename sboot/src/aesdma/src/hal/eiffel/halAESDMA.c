...
{
    //RSA interrupt clear
    RIU[(RSA_BASE_ADDR+(0x77<<1))]= ((RIU[(RSA_BASE_ADDR+(0x77<<1))])|(RSA_INT_CLR));
}

void HAL_RSA_Reset(void)
{
    //RSA Rst
    RIU[(RSA_BASE_ADDR+(0x78<<1))]= ((RIU[(RSA_BASE_ADDR+(0x78<<1))])|(RSA_CTRL_RSA_RST));
    RIU[(RSA_BASE_ADDR+(0x78<<1))]= ((RIU[(RSA_BASE_ADDR+(0x78<<1))])&(~RSA_CTRL_RSA_RST));
}
...
void HAL_RSA_Ind32Ctrl(U8 u8dirction)
{
    ...
    if(u8dirction==1)
    {
        RIU[(RSA_BASE_ADDR+(0x71<<1))]= ((RIU[(RSA_BASE_ADDR+(0x71<<1))])|(RSA_IND32_CTRL_DIRECTION_WRITE));
    }
    else
    {
        RIU[(RSA_BASE_ADDR+(0x71<<1))]= ((RIU[(RSA_BASE_ADDR+(0x71<<1))])&(~RSA_IND32_CTRL_DIRECTION_WRITE));
    ...
    RIU[(RSA_BASE_ADDR+(0x71<<1))]= ((RIU[(RSA_BASE_ADDR+(0x71<<1))])|(RSA_IND32_CTRL_ADDR_AUTO_INC));

    //[3] Set 1 to enable access auto-start after writing Data[31:16]
    RIU[(RSA_BASE_ADDR+(0x71<<1))]= ((RIU[(RSA_BASE_ADDR+(0x71<<1))])|(RSA_IND32_CTRL_ACCESS_AUTO_START));
...
void HAL_RSA_LoadSiganature(U32 *ptr_Sign)
{
    ...
    RIU[(RSA_BASE_ADDR+(0x72<<1))]= RSA_A_BASE_ADDR; //RSA A addr
    RIU[(RSA_BASE_ADDR+(0x70<<1))]= ((RIU[(RSA_BASE_ADDR+(0x70<<1))])|(RSA_IND32_START)); //RSA start

    for( i = 0; i < 64; i++ )
    {
        RIU[(RSA_BASE_ADDR+(0x73<<1))]= (U16)(((*(ptr_Sign-i))>>8)&0xFF00)|(((*(ptr_Sign-i))>>24)&0xFF);
        RIU[(RSA_BASE_ADDR+(0x74<<1))]= (U16)(((*(ptr_Sign-i))>>8)&0xFF)|(((*(ptr_Sign-i))<<8)&0xFF00);
    }
...
void HAL_RSA_LoadKeyE(U32 *ptr_E)
{
    ...
    RIU[(RSA_BASE_ADDR+(0x72<<1))]= RSA_E_BASE_ADDR; //RSA E addr
    RIU[(RSA_BASE_ADDR+(0x70<<1))]= ((RIU[(RSA_BASE_ADDR+(0x70<<1))])|(RSA_IND32_START)); //RSA start
    ...
    for( i = 0; i < 64; i++ )
    {
        RIU[(RSA_BASE_ADDR+(0x73<<1))]= (U16)(((*(ptr_E+i))>>8)&0xFF00)|(((*(ptr_E+i))>>24)&0xFF);
        RIU[(RSA_BASE_ADDR+(0x74<<1))]= (U16)(((*(ptr_E+i))>>8)&0xFF)|(((*(ptr_E+i))<<8)&0xFF00);
    }
...
void HAL_RSA_LoadKeyN(U32 *ptr_N)
{
    ...
    RIU[(RSA_BASE_ADDR+(0x72<<1))]= RSA_N_BASE_ADDR; //RSA N addr
    RIU[(RSA_BASE_ADDR+(0x70<<1))]= ((RIU[(RSA_BASE_ADDR+(0x70<<1))])|(RSA_IND32_START)); //RSA start
    ...
    for( i = 0; i < 64; i++ )
    {
        RIU[(RSA_BASE_ADDR+(0x73<<1))]= (U16)(((*(ptr_N+i))>>8)&0xFF00)|(((*(ptr_N+i))>>24)&0xFF);
        RIU[(RSA_BASE_ADDR+(0x74<<1))]= (U16)(((*(ptr_N+i))>>8)&0xFF)|(((*(ptr_N+i))<<8)&0xFF00);
    }
...
void HAL_RSA_SetKeyLength(U16 u16keylen)
{
    ...
    RIU[(RSA_BASE_ADDR+(0x78<<1))] = ((RIU[(RSA_BASE_ADDR+(0x78<<1))])|(u16keylen<<8));
}
...
void HAL_RSA_SetKeyType(U8 u8hwkey, U8 u8pubkey)
{
    ...
    if(u8hwkey==1)
    {
        RIU[(RSA_BASE_ADDR+(0x78<<1))] = ((RIU[(RSA_BASE_ADDR+(0x78<<1))])|(RSA_SEL_HW_KEY));
    }
    else
    {
        RIU[(RSA_BASE_ADDR+(0x78<<1))] = ((RIU[(RSA_BASE_ADDR+(0x78<<1))])&(~RSA_SEL_HW_KEY));
    ...
    if(u8pubkey==1)
    {
        RIU[(RSA_BASE_ADDR+(0x78<<1))] = ((RIU[(RSA_BASE_ADDR+(0x78<<1))])|(RSA_SEL_PUBLIC_KEY));
    }
    else
    {
        RIU[(RSA_BASE_ADDR+(0x78<<1))] = ((RIU[(RSA_BASE_ADDR+(0x78<<1))])&(~RSA_SEL_PUBLIC_KEY));
...
{
    //RSA exp start
    RIU[(RSA_BASE_ADDR+(0x77<<1))] = ((RIU[(RSA_BASE_ADDR+(0x77<<1))])|(RSA_EXP_START));
}

U16 HAL_RSA_GetStatus(void)
{
    return RIU[(RSA_BASE_ADDR+(0x79<<1))];
}

void HAL_RSA_FileOutStart(void)
{
    //RSA ind32_start
    RIU[(RSA_BASE_ADDR+(0x70<<1))]= ((RIU[(RSA_BASE_ADDR+(0x70<<1))])|(RSA_IND32_START));
}

void HAL_RSA_SetFileOutAddr(U32 u32offset)
{
    //RSA ind32_addr
    RIU[(RSA_BASE_ADDR+(0x72<<1))] = (U16)(RSA_Z_BASE_ADDR + u32offset);
}

U32 HAL_RSA_FileOut(void)
{
    return (U32)(RIU[(RSA_BASE_ADDR+(0x75<<1))]|(RIU[(RSA_BASE_ADDR+(0x76<<1))]<<16));
}

void HAL_SHA_Reset(void)
{
    //SHA_Reset
    RIU[SHARNG_BASE_ADDR+(0x58<<1)]= (RIU[SHARNG_BASE_ADDR+(0x58<<1)]|(SHARNG_CTRL_SHA_RST));
    RIU[SHARNG_BASE_ADDR+(0x58<<1)]= (RIU[SHARNG_BASE_ADDR+(0x58<<1)]&(~SHARNG_CTRL_SHA_RST));
}
...
{
    //SHA_SetLength:5c~5d(sha_message_length)
    RIU[(SHARNG_BASE_ADDR+(0x5C<<1))]= (U16)((0x0000ffff)&(u32Size));
    RIU[(SHARNG_BASE_ADDR+(0x5D<<1))]= (U16)(((0xffff0000)&(u32Size))>>16);
}
...
void HAL_SHA_SelMode(U8 u8sha256)
{
    ...
    if(u8sha256==1)
    {
        RIU[(SHARNG_BASE_ADDR+(0x58<<1))]= ((RIU[(SHARNG_BASE_ADDR+(0x58<<1))])|(SHARNG_CTRL_SHA_SEL_SHA256));
    }
    else
    {
        RIU[(SHARNG_BASE_ADDR+(0x58<<1))]= ((RIU[(SHARNG_BASE_ADDR+(0x58<<1))])&(~SHARNG_CTRL_SHA_SEL_SHA256));
...
{
    return (RIU[(SHARNG_BASE_ADDR+(0x5F<<1))]);
}

void HAL_SHA_Clear(void)
{
    //Set "1" to idle state after reg_read_sha_ready = 1
    RIU[(SHARNG_BASE_ADDR+(0x58<<1))]= ((RIU[(SHARNG_BASE_ADDR+(0x58<<1))])&(~SHARNG_CTRL_SHA_CLR));
    RIU[(SHARNG_BASE_ADDR+(0x58<<1))]= ((RIU[(SHARNG_BASE_ADDR+(0x58<<1))])|(SHARNG_CTRL_SHA_CLR));
}
...
void HAL_SHA_Out(U32 u32Buf)
{
    ...
    {
        *((U16 *)u32Buf + index) = (RIU[(SHARNG_BASE_ADDR+(0x60<<1)+index*2)]);
    }
}