...
typedef enum
{
    ...
    VDEC_USER_CMD_SET_TIME_INC_PREDICT_PARA,        // Param: Arg0(enable), Arg1( 1 for predict once), Arg2(0 for follow vol_header)
    VDEC_USER_CMD_ERR_CONCEAL_SLICE_1ST_MB,         // Param: 0(disable), Error concealment from current/last MB position; 1(enale) Error concealment from current slice first MB.(Need enable E_HVD_CMD_ERR_CONCEAL)
    VDEC_USER_CMD_RETURN_INVALID_AFD,               // Param: 0(Disable), 1(Enable)
    VDEC_USER_CMD_AUTO_FREE_ES,                     // Param: 0(Disable), 1(Enable)
    ...
    VDEC_USER_CMD_SET_ERROR_TOLERANCE,              // Param: the address of VDEC_Err_Tolerance, bEnable:enable/disable,  u8Tolerance: err_rate(0%~100%))
	...
	VDEC_USER_CMD_AVC_ENABLE_NEW_SLOW_MOTION, 		// Param: 0(Disable), 1(Enable)
    ...
    VDEC_USER_CMD_GET_U64PTS,                       // Get 64 bits pts
    ...
    VDEC_USER_CMD_CORRECT_ERROR_FRAMERATE,
...
//-------------------------------------------------------------------------------------------------
DLL_PUBLIC VDEC_Result MApi_VDEC_GetLibVer(const MSIF_Version **ppVersion);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetInfo(const VDEC_Info **ppInfo);
DLL_PUBLIC MS_BOOL     MApi_VDEC_GetStatus(VDEC_Status *pStatus);
DLL_PUBLIC MS_BOOL     MApi_VDEC_CheckCaps(VDEC_CodecType eCodecType);
DLL_PUBLIC VDEC_Result MApi_VDEC_EnableTurboMode(MS_BOOL bEnable);
DLL_PUBLIC VDEC_Result MApi_VDEC_Init(VDEC_InitParam *pInitParam);
DLL_PUBLIC VDEC_Result MApi_VDEC_Init_EX(void *pInfo, MS_U32 u32Size);
DLL_PUBLIC VDEC_Result MApi_VDEC_Rst(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_Exit(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_CheckDispInfoRdy(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetFrcMode(VDEC_FrcMode eFrcMode);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetDynScalingParams( MS_PHYADDR u32Addr , MS_U32 u32Size);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetDbgLevel(VDEC_DbgLevel eDbgLevel);


DLL_PUBLIC VDEC_Result MApi_VDEC_Play(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_Pause(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_Resume(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_StepDisp(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsStepDispDone(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_StepDecode(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsStepDecodeDone(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetTrickMode(VDEC_TrickDec eTrickDec);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetBlockDisplay(MS_BOOL bEnable);

DLL_PUBLIC VDEC_Result MApi_VDEC_EnableESBuffMalloc(MS_BOOL bEnable);
DLL_PUBLIC MS_U32 MApi_VDEC_GetESBuffVacancy(void* pData);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetESBuff(MS_U32 u32ReqSize, MS_U32* u32AvailSize, MS_PHYADDR* u32Addr);
...
DLL_PUBLIC VDEC_Result MApi_VDEC_PushDecQ(VDEC_DecCmd *pCmd);
DLL_PUBLIC VDEC_Result MApi_VDEC_PushDispQ(VDEC_DispCmd *pCmd);
DLL_PUBLIC VDEC_Result MApi_VDEC_Flush(VDEC_FreezePicSelect eFreezePic);
DLL_PUBLIC VDEC_Result MApi_VDEC_EnableLastFrameShow(MS_BOOL bEnable);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetSpeed(VDEC_SpeedType eSpeedType, VDEC_DispSpeed eSpeed);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetFreezeDisp(MS_BOOL bEnable);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetBlueScreen(MS_BOOL bOn);
DLL_PUBLIC VDEC_Result MApi_VDEC_ResetPTS(MS_U32 u32PtsBase);
DLL_PUBLIC VDEC_Result MApi_VDEC_AVSyncOn(MS_BOOL bOn, MS_U32 u32SyncDelay, MS_U16 u16SyncTolerance);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetAVSyncFreerunThreshold(MS_U32 u32Threshold );

DLL_PUBLIC VDEC_Result MApi_VDEC_GetDispInfo(VDEC_DispInfo *pDispinfo);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsAVSyncOn(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsWithValidStream(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsDispFinish(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsFrameRdy(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsIFrameFound(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsSeqChg(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsReachSync(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsFreerun(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsStartSync(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsWithLowDelay(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsAllBufferEmpty(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetExtDispInfo(VDEC_ExtDispInfo *pExtDispinfo);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetDecFrameInfo(VDEC_FrameInfo *pFrmInfo);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetDispFrameInfo(VDEC_FrameInfo *pFrmInfo);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetNextDispFrame(VDEC_DispFrame** ppDispFrm);
DLL_PUBLIC VDEC_Result MApi_VDEC_DisplayFrame(VDEC_DispFrame *pDispFrm);
DLL_PUBLIC VDEC_Result MApi_VDEC_ReleaseFrame(VDEC_DispFrame *pDispFrm);

DLL_PUBLIC VDEC_Result MApi_VDEC_GetDecTimeCode(VDEC_TimeCode* pTimeCode);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetDispTimeCode(VDEC_TimeCode* pTimeCode);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetEvent(MS_U32 u32EnableEvent, VDEC_EventCb pfn, void* param);
DLL_PUBLIC VDEC_Result MApi_VDEC_UnsetEvent(MS_U32 u32EnableEvent);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetEvent_EX(VDEC_CB_TYPE cb_type, MS_U32 u32EnableEvent, VDEC_EventCb pfn, void* param);
DLL_PUBLIC VDEC_Result MApi_VDEC_UnsetEvent_EX(VDEC_CB_TYPE cb_type, MS_U32 u32EnableEvent);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetEventInfo(MS_U32* u32EventFlag);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetPatchPattern(VDEC_PatchPattern ePatternType, MS_U32 *pu32Staddr, MS_U32 *pu32Len);
DLL_PUBLIC VDEC_Result MApi_VDEC_FireDecCmd(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_SeekToPTS(MS_U32 u32PTS);
DLL_PUBLIC VDEC_Result MApi_VDEC_SkipToPTS(MS_U32 u32PTS);

// Advanced Settings / Mode
DLL_PUBLIC VDEC_Result MApi_VDEC_DisableDeblocking(MS_BOOL bDisable);
DLL_PUBLIC VDEC_Result MApi_VDEC_DisableQuarterPixel(MS_BOOL bDisable);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetAutoRmLstZeroByte(MS_BOOL bOn);
DLL_PUBLIC VDEC_Result MApi_VDEC_SetBalanceBW(MS_U8 u8QuarPixelTH, MS_U8 u8DeBlockingTH, MS_U8 u8UpperBound, MS_U8 u8SafeCoef);

DLL_PUBLIC MS_U8       MApi_VDEC_GetActiveFormat(void);
DLL_PUBLIC MS_U8       MApi_VDEC_GetColourPrimaries(void);
DLL_PUBLIC MS_U8       MApi_VDEC_GetGOPCnt(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetESWritePtr(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetESReadPtr(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetPTS(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetNextPTS(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetErrCode(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetErrCnt(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetBitsRate(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetFrameCnt(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetSkipCnt(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetDropCnt(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetDecQVacancy(void);
DLL_PUBLIC MS_U32      MApi_VDEC_GetDispQVacancy(void);
DLL_PUBLIC MS_BOOL     MApi_VDEC_Is32PullDown(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_IsAlive(void);
DLL_PUBLIC MS_S64      MApi_VDEC_GetVideoPtsStcDelta(void);
DLL_PUBLIC MS_BOOL     MApi_VDEC_IsCCAvailable(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_GetCCInfo(void *pInfo, MS_U32 u32Size);

DLL_PUBLIC VDEC_TrickDec MApi_VDEC_GetTrickMode(void);
DLL_PUBLIC VDEC_CodecType MApi_VDEC_GetActiveCodecType(void);

DLL_PUBLIC VDEC_Result MDrv_VDEC_GenPattern(VDEC_PatternType ePatternType ,  MS_PHYADDR u32Addr , MS_U32* u32Size);
DLL_PUBLIC VDEC_Result MApi_VDEC_GenPattern(VDEC_PatternType ePatternType, MS_PHYADDR u32Addr, MS_U32 *pu32Size);
DLL_PUBLIC MS_U32 MApi_VDEC_GetPatternLeastLength(VDEC_PatternType ePatternType );

DLL_PUBLIC VDEC_Result MApi_VDEC_MHEG_DecodeIFrame(MS_PHYADDR u32FrameBufAddr,
                                        MS_PHYADDR u32SrcSt,
                                        MS_PHYADDR u32SrcEnd);
DLL_PUBLIC VDEC_Result MApi_VDEC_MHEG_IsIFrameDecoding(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_MHEG_RstIFrameDec(void);

//ATSC Closed Caption control
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_StartParsing(VDEC_CCCfg *pCCParam);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_StopParsing(void);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_GetWritePtr(MS_U32 *pWrite);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_GetReadPtr(MS_U32 *pRead);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_UpdateReadPtr(MS_U32 u32EachPacketSize);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_GetIsOverflow(MS_BOOL *pbOverflow);

DLL_PUBLIC VDEC_Result MApi_VDEC_CC_Init(MS_U32 *pIn);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_SetCfg(VDEC_CCFormat eFmt, VDEC_CCType eType, MS_U32 *pIn);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_SetBuffStartAdd(VDEC_CCFormat eFmt, MS_U32 *pIn);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_UpdateWriteAdd(VDEC_CCFormat eFmt, MS_U32 *pIn);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_UpdateReadAdd(VDEC_CCFormat eFmt, MS_U32 *pIn);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_DisableParsing(VDEC_CCFormat eFmt);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_GetIsRstDone(VDEC_CCFormat eFmt);
DLL_PUBLIC VDEC_Result MApi_VDEC_CC_GetIsBuffOverflow(VDEC_CCFormat eFmt);
DLL_PUBLIC MS_PHYADDR MApi_VDEC_CC_GetWriteAdd(VDEC_CCFormat eFmt);
DLL_PUBLIC MS_PHYADDR MApi_VDEC_CC_GetReadAdd(VDEC_CCFormat eFmt);

DLL_PUBLIC VDEC_Result MApi_VDEC_DbgCmd(MS_U32 u8Cmd, MS_U32 u32CmdArg);
DLL_PUBLIC VDEC_Result MApi_VDEC_DbgSetData(MS_U32 u32Addr, MS_U32 u32Data);
DLL_PUBLIC VDEC_Result MApi_VDEC_DbgGetData(MS_U32 u32Addr, MS_U32 *u32Data);

DLL_PUBLIC VDEC_Result MApi_VDEC_SetControl(VDEC_User_Cmd cmd_id, MS_U32 param);

DLL_PUBLIC MS_U32      MApi_VDEC_GetFwVersion(VDEC_FwType eFwType);

DLL_PUBLIC MS_U32 MApi_VDEC_GetDispCnt(void);

DLL_PUBLIC VDEC_Result MApi_VDEC_GetControl(VDEC_User_Cmd cmd_id, MS_U32 *param);
DLL_PUBLIC VDEC_Result MApi_VDEC_FlushPTSBuf(void);

//System preseting function, it should be called when system init.
DLL_PUBLIC VDEC_Result MApi_VDEC_SystemPreSetControl(VDEC_User_Cmd cmd_id, void *pParam);

//Pre-Setting function, it should be called just before vdec init.
DLL_PUBLIC VDEC_Result MApi_VDEC_PreSetControl(VDEC_User_Cmd cmd_id, MS_U32 param);
