...
DLL_PUBLIC MS_BOOL  MApi_CEC_GetLibVer(const MSIF_Version **ppVersion);      ///< Get version (without Mutex protect)
DLL_PUBLIC MS_BOOL  MApi_DDC2BI_GetInfo(MS_CEC_INFO_LIST *pInfo);
DLL_PUBLIC MS_BOOL  MApi_DDC2BI_GetStatus(MS_CEC_Status *pStatus);
DLL_PUBLIC MS_BOOL  MApi_DDC2BI_SetDbgLevel(MS_U16 u16DbgSwitch);

DLL_PUBLIC void MApi_CEC_Exit(void);

//                  CEC initial sequence
DLL_PUBLIC void MApi_CEC_Init(MS_CEC_INIT_INFO *pCECInfo);

//                  CEC initial chip relatived part
DLL_PUBLIC void MApi_CEC_InitChip(MS_CEC_INIT_INFO *pCECInfo);

//                  CEC set own logical address
DLL_PUBLIC void MApi_CEC_SetMyLogicalAddress(MsCEC_DEVICELA myLA);

//                  CEC set retry count
DLL_PUBLIC void MApi_CEC_SetRetryCount(MS_U8 u8RetryCount);

//                  Use to store CEC received messages
DLL_PUBLIC void MApi_CEC_ChkRxBuf(void);


//                   Use to send any CEC message by user defined
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_TxSendMsg(MsCEC_DEVICELA header,MsCEC_MSGLIST opcode,MS_U8* operand_ptr,MS_U8 len);

//                   Use to send any CEC message by user defined
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_TxSendMsg2(MsCEC_DEVICELA dst_address, MsCEC_MSGLIST msg, MS_U8* operand_ptr, MS_U8 len);

//                   Use to send CEC polling msg
DLL_PUBLIC CEC_ERROR_CODE MsAPI_CecTxSendPollingMsg(MsCEC_DEVICELA dst_address);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ActiveSource(void);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_RoutingChange(MS_U8* orig_address, MS_U8* new_address);

//                   Used by a new device to discover the status of the system.
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReqActiveSource(void);

//                   Used by the TV to request a streaming path from the specified physical address.
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_SetStreamPath(MS_U8* new_address);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_Standby(MsCEC_DEVICELA dst_address);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_RecordOff(MsCEC_DEVICELA dst_address);

//                   Attempt to record the specified source.
DLL_PUBLIC CEC_ERROR_CODE MsAPI_CEC_Msg_RecordOn(MsCEC_DEVICELA dst_address, MsCEC_MSG_RECORD_SOURCE_TYPE cmd);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReportCECVersion(MsCEC_DEVICELA dst_address, MS_U8 version);

//                   Used by a device to enquire which version of CEC the target supports
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReqCECVersion(MsCEC_DEVICELA dst_address);

//                   Use to inform all other devices of the mapping between physical and logical address of the initiator
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReportPhycalAddress(void);

//                   A request to a device to return its physical address
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReqPhycalAddress(MsCEC_DEVICELA dst_address);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_DeckControl(MsCEC_DEVICELA dst_address, MsCEC_MSG_DC_DECK_CTRL_PARM cmd);

//                   Use to provide a device's status when received "Give Deck Status"
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_DecStatus(MsCEC_DEVICELA dst_address, MsCEC_MSG_DC_DECK_STATUS_PARM cmd);

//                   Use to request device's status
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_MSg_GiveDeckStatus(MsCEC_DEVICELA dst_address, MsCEC_MSG_STATUS_REQUEST_PARM cmd);

//                   Use to control the playback behaviour of a source device
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_MSg_DCPlay(MsCEC_DEVICELA dst_address, MsCEC_MSG_DC_PLAY_PARM cmd);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReqMenuStatus(MsCEC_DEVICELA dst_address, MsCEC_MSG_MENU_REQ_TYPE cmd);

//                   Use to indicated that user pressed which control button
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_UserCtrlPressed(MS_BOOL bUserCtrlEn, MsCEC_DEVICELA dst_address, MsCEC_MSG_USER_CTRL_PARM cmd);

//                   Use to indicated that user released which control button
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_UserCtrlReleased(MsCEC_DEVICELA dst_address);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReportPowerStatus(MsCEC_DEVICELA dst_address);


//                   Use to determine the current power status of a target device
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReqPowerStatus(MsCEC_DEVICELA dst_address);
...
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_FeatureAbort(MsCEC_DEVICELA dst_address, MsCEC_MSGLIST msg, MsCEC_MSG_ABORT_REASON cmd);

//                   This message is reserved for testing purposes.
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_Abort(MsCEC_DEVICELA dst_address);

//
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_SendMenuLanguage(MS_U8* pu8MenuLanguageCode);

DLL_PUBLIC CEC_ERROR_CODE MsAPI_CecMsg_ReqARCInitiation(MsCEC_DEVICELA dst_address);
DLL_PUBLIC CEC_ERROR_CODE MsAPI_CecMsg_ReqARCTermination(MsCEC_DEVICELA dst_address);
DLL_PUBLIC CEC_ERROR_CODE MsAPI_CecMsg_AudioModeReq(MsCEC_DEVICELA dst_address, MS_BOOL amp_switch, MS_U8* my_address );
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_GiveAudioStatus(MsCEC_DEVICELA dst_address);

DLL_PUBLIC MS_U8 MApi_CEC_Get_Header(void);
DLL_PUBLIC MS_U8 MApi_CEC_Get_OpCode(void);
DLL_PUBLIC MS_U8 MApi_CEC_Get_Para(MS_U8 u8Idx);
DLL_PUBLIC MS_U8 MApi_CEC_GetCmdLen(void);
DLL_PUBLIC void MApi_CEC_SetActiveLogicalAddress(MsCEC_DEVICELA Addr);
DLL_PUBLIC MsCEC_DEVICELA MApi_CEC_GetActiveLogicalAddress(void);

DLL_PUBLIC MsCEC_MSG_POWER_STATUS_PARM MApi_CEC_GetPowerStatus(void);
DLL_PUBLIC MS_U8 MApi_CEC_GetFifoIdx(void);
DLL_PUBLIC void MApi_CEC_SetFifoIdx(MS_U8 u8Idx);

DLL_PUBLIC void MApi_CEC_SetActivePowerStatus(MsCEC_MSG_POWER_STATUS_PARM Status);
DLL_PUBLIC MsCEC_MSG_POWER_STATUS_PARM MApi_CEC_GetActivePowerStatus(void);

DLL_PUBLIC void MApi_CEC_SetActivePhysicalAddress(MS_U8 u8Para1, MS_U8 u8Para2);
DLL_PUBLIC void MApi_CEC_SetActiveDeviceCECVersion(MS_U8 u8Ver);
DLL_PUBLIC void MApi_CEC_SetActiveDeviceType(MsCEC_DEVICE_TYPE Type);

DLL_PUBLIC MS_U8 MApi_CEC_GetMsgCnt(void);
DLL_PUBLIC void MApi_CEC_SetMsgCnt(MS_U8 u8Cnt);

DLL_PUBLIC MS_U8 MApi_CEC_GetRxData(MS_U8 u8Fifoidx, MS_U8 u8Idx);
DLL_PUBLIC MS_BOOL MApi_CEC_CheckFrame(MsCEC_MSG_TRANS_TYPE msg_type, MS_U8 ucLen);
DLL_PUBLIC void MApi_CEC_CheckExistDevices(void);
DLL_PUBLIC void MApi_CEC_ConfigWakeUp(void);
DLL_PUBLIC void MApi_CEC_Enabled(MS_BOOL bEnableFlag);
DLL_PUBLIC void MApi_CEC_SetMyPhysicalAddress(MS_U8 *pdata);
#if ENABLE_CEC_MULTIPLE
DLL_PUBLIC void MApi_CEC_SetMyLogicalAddress2(MsCEC_DEVICELA myLA);
DLL_PUBLIC CEC_ERROR_CODE MApi_CEC_Msg_ReportPhycalAddress2(void);
DLL_PUBLIC void MApi_CEC_SetMyPhysicalAddress2(MS_U8 *pdata);
DLL_PUBLIC void MApi_CEC_SetInitiator(MsCEC_DEVICELA IniLA);
#endif
...
DLL_PUBLIC MS_U8 MApi_CEC_GetTxStatus(void);
DLL_PUBLIC MS_CEC_INFO_LIST MApi_CEC_GetInfo(void);
