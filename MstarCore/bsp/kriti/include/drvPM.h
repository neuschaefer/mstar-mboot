...
#define PM_MAX_IR_SUP                  (5)
#define PM_MAX_BUF_WAKE_IR_KEY         (6)
...
#define PM_MAX_BUF_WAKE_IR_WAVEFORM_ADDR    (4)//For decoding IR waveform
...
typedef enum
{
    ...
    E_PM_ACKFLG_WAIT_IRCONFIG = (1<<19),            /// Ack flag for Set IR config
} PM_AckFlags;
...
{
    MS_U8  u8IrNum;
    MS_U8  u8IrMode[PM_MAX_IR_SUP]; //0: 32bit ir, 1:48bit ir
    MS_U16 u16Reserved;
    MS_U32 u32CustomerCode[PM_MAX_IR_SUP];
    MS_U16 u16WakeKeyCode[PM_MAX_IR_SUP][PM_MAX_BUF_WAKE_IR_KEY];
} PM_IrCfg;
...
//-------------------------------------------------------------------------------------------------
DLL_PUBLIC MS_BOOL MDrv_PM_isRunning(void);
DLL_PUBLIC void MDrv_PM_InterruptRequest( void );
DLL_PUBLIC void MDrv_PM_LoadFw( MS_U8 *pPmCode, MS_U16 u16Len );
DLL_PUBLIC MS_BOOL MDrv_PM_RegWrite( MS_U16 u16Addr, MS_U8 u8Data );
DLL_PUBLIC MS_U8 MDrv_PM_RegRead( MS_U16 u16Addr );
DLL_PUBLIC void MDrv_PM_RegWriteBit(MS_U16 u16Addr, MS_U8 bBit, MS_U8 u8BitPos );
DLL_PUBLIC MS_U8 MDrv_PM_RegReadBit(MS_U16 u16Addr, MS_U8 u8BitPos);
DLL_PUBLIC void MDrv_PM_RegWrite2byte( MS_U16 u16RegIndex, MS_U16 u16Value );
DLL_PUBLIC void MDrv_PM_RegWrite3byte( MS_U16 u16Regndex, MS_U32 u32Value );
DLL_PUBLIC MS_BOOL MDrv_PM_Control( MS_U8 u8opcode, MS_U8 u8Data, MS_U16 u16Data );
DLL_PUBLIC MS_BOOL MDrv_PM_CtrlWrite( MS_U8 u8Opcode, MS_U8 u8CmdType, MS_U8 u8DataNum, MS_U8 *pu8Data );
DLL_PUBLIC MS_BOOL MDrv_PM_CtrlRead( MS_U8 u8Opcode, MS_U8 u8CmdType, MS_U8 u8DataNum, MS_U8 *pu8Data );
DLL_PUBLIC MS_BOOL MDrv_PM_MemoryWrite(MS_U8 *pu8Data, MS_U16 u16MemLen);
DLL_PUBLIC MS_BOOL MDrv_PM_MemoryRead(MS_U8 *pu8Data, MS_U16 u16MemLen);
DLL_PUBLIC void MDrv_PM_RTCEnableInterrupt(MS_BOOL bEnable);
DLL_PUBLIC void MDrv_PM_RTCSetMatchTime(MS_U32 u32PmSysTime);
DLL_PUBLIC MS_U32 MDrv_PM_RTCGetMatchTime(void);
DLL_PUBLIC void MDrv_PM_RTCSetSystemTime(MS_U32 u32PmSysTime);
DLL_PUBLIC MS_U32 MDrv_PM_RTCGetSystemTime(void);
DLL_PUBLIC void MDrv_PM_RTCInit(MS_U32 u32CtrlWord);
DLL_PUBLIC void MDrv_PM_IRInit(MS_U8 irclk_mhz, PM_IrRegCfg *irRegCfg);
DLL_PUBLIC void MDrv_PM_KeypadInit(PM_SarRegCfg *sarRegCfg, PM_SarParamCfg *sarParamCfg);
DLL_PUBLIC void MDrv_PM_CalibrateRC(void);
DLL_PUBLIC MS_BOOL MDrv_PM_isDownloaded(void);
DLL_PUBLIC void MDrv_PM_Set_Download(void);

DLL_PUBLIC void MDrv_PM_IrqAttach(PM_IRQ_TYPE irq, PM_IsrCb_Type isr, MS_U32 data);
DLL_PUBLIC void MDrv_PM_IrqDetach(PM_IRQ_TYPE irq);
DLL_PUBLIC void MDrv_PM_IrqUnmask(PM_IRQ_TYPE irq);
DLL_PUBLIC void MDrv_PM_IrqMask(PM_IRQ_TYPE irq);
DLL_PUBLIC void MDrv_PM_IrqMaskAll(void);

DLL_PUBLIC PM_Result MDrv_PM_ActiveStandbyMode(MS_BOOL bTrigger);
DLL_PUBLIC PM_Result MDrv_PM_IsActiveStandbyMode(MS_BOOL *bActive);

DLL_PUBLIC PM_Result MDrv_PM_Init(PM_WakeCfg *pPmWakeCfg);
DLL_PUBLIC PM_Result MDrv_PM_IrConfig(PM_IrCfg *pPmIrCfg);
DLL_PUBLIC PM_Result MDrv_PM_GetStatus(PM_DrvStatus *pDrvStatus);
DLL_PUBLIC PM_Result MDrv_PM_GetLibVer(const MSIF_Version **ppVersion);
DLL_PUBLIC PM_Result MDrv_PM_PowerDown(PM_PowerDownCfg *pPmPowerDownCfg);
DLL_PUBLIC PM_Result MDrv_PM_GetMasterKey(MS_U8 *pPmPowerDownCfg);
DLL_PUBLIC PM_Result MDrv_PM_GetDeviceID(MS_U8 *DeviceID);
DLL_PUBLIC PM_Result MDrv_PM_GetChipID(MS_U8 *ChipParam);
DLL_PUBLIC PM_Result MDrv_PM_GetPMMemAddr(MS_U8 *PmMemAddr);
DLL_PUBLIC const PM_DrvInfo* MDrv_PM_GetInfo(void);
DLL_PUBLIC PM_Result MDrv_PM_GPIOInit(MS_U16 u16GPIOIndex);
DLL_PUBLIC PM_Result MDrv_PM_SetDbgLevel(PM_DbgLv eLevel);
DLL_PUBLIC PM_Result MDrv_PM_RtcInit(PM_RtcParam *pPmRtcParam);
DLL_PUBLIC void MDrv_PM_Rtc_DisableInit(E_PM_RTC eRtc);
DLL_PUBLIC PM_Result MDrv_PM_RtcSetCounter(PM_RtcParam *pPmRtcParam);
DLL_PUBLIC PM_Result MDrv_PM_RtcGetCounter(PM_RtcParam *pPmRtcParam);
DLL_PUBLIC PM_Result MDrv_PM_RtcSetMatchCounter(PM_RtcParam *pPmRtcParam);
DLL_PUBLIC PM_Result MDrv_PM_RtcGetMatchCounter(PM_RtcParam *pPmRtcParam);
DLL_PUBLIC PM_Result MDrv_PM_GetExtraSramData(PM_ExtraSramData *pPmExtraSramData);
DLL_PUBLIC PM_Result MDrv_PM_SetExtraSramData(PM_ExtraSramData *pPmExtraSramData);

DLL_PUBLIC void MDrv_PM_SetSPIOffsetForMCU(MS_U32 BANK);
DLL_PUBLIC void MDrv_PM_SetSRAMOffsetForMCU(void);
DLL_PUBLIC void MDrv_PM_SetDRAMOffsetForMCU(MS_U32 u32Offset);

DLL_PUBLIC PM_PowerOnMode MDrv_PM_PowerOnMode(void);
DLL_PUBLIC PM_WakeupSource MDrv_PM_GetWakeupSource(void);
DLL_PUBLIC MS_U8 MDrv_PM_GetWakeupKey(void);
DLL_PUBLIC PM_Result MDrv_PM_Disable51(void);

DLL_PUBLIC PM_Result MDrv_PM_GPIO4_SetPower(MS_BOOL bOn);

DLL_PUBLIC PM_Result MDrv_PM_PWM_Init(MS_U8 u8PWM);
DLL_PUBLIC PM_Result MDrv_PM_PWM_Config(PWMSimIR_CFG *pPmPWMCfg);

DLL_PUBLIC PM_Result MDrv_PM_PWM_IRRecord_Init(void);
DLL_PUBLIC PM_Result MDrv_PM_PWM_IRRecord_Receive_Complete(MS_U32 u32BufferAddr,MS_U16 u16BufferSize);
DLL_PUBLIC PM_Result MDrv_PM_PWM_IRRecord_Receive_Require(MS_U32 u32BufferAddr,MS_U16 u16BufferSize);
DLL_PUBLIC PM_Result MDrv_PM_PWM_IRRecord_Transmit(MS_U32 u32BufferAddr,MS_U16 u16BufferSize);
DLL_PUBLIC PM_Result MDrv_PM_PWM_IRRecord_SetCallBackFunction(IRRecord_Callback pCallback);
DLL_PUBLIC PM_Result MDrv_PM_PWM_IRRecord_Receive_Exit(void);

DLL_PUBLIC void MDrv_PM_RunTimePM_Disable_PassWord(void);
DLL_PUBLIC void MDrv_PM_STR_CheckFactoryPowerOnModePassword(void);
DLL_PUBLIC PM_Result MDrv_PM_STR_CheckFactoryPowerOnMode_Second(MS_BOOL bCheck);

DLL_PUBLIC PM_Result MDrv_PM_Check_Version(MS_BOOL bCheck);
DLL_PUBLIC MS_U16 MDrv_PM_GetSRAMSize(void);
DLL_PUBLIC MS_U8 MDrv_PM_GetIRPowerOnKey(void);
#ifdef __cplusplus
}
#endif
