...
#endif
  , E_JPEG_BAD_APP3_MARKER              = -251
  , E_JPEG_BAD_APP14_MARKER             = -262
...
DLL_PUBLIC void msAPI_JPEG_get_APP0_info( MS_U8 *unit,MS_U16 *x, MS_U16 *y );

DLL_PUBLIC JPEG_Result MApi_JPEG_Init(JPEG_InitParam *pInitParam);
DLL_PUBLIC JPEG_Result MApi_JPEG_DecodeHdr(void);
DLL_PUBLIC JPEG_Result MApi_JPEG_Decode(void);
DLL_PUBLIC void MApi_JPEG_Exit(void);
DLL_PUBLIC JPEG_ErrCode MApi_JPEG_GetErrorCode(void);
DLL_PUBLIC JPEG_Event MApi_JPEG_GetJPDEventFlag(void);
DLL_PUBLIC void MApi_JPEG_SetJPDEventFlag(JPEG_Event eEvtVal);
DLL_PUBLIC void MApi_JPEG_Rst(void);
DLL_PUBLIC void MApi_JPEG_PowerOn(void);
DLL_PUBLIC void MApi_JPEG_PowerOff(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetCurVidx(void);
DLL_PUBLIC MS_BOOL MApi_JPEG_IsProgressive(void);
DLL_PUBLIC MS_BOOL MApi_JPEG_ThumbnailFound(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetWidth(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetHeight(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetOriginalWidth(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetOriginalHeight(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetNonAlignmentWidth(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetNonAlignmentHeight(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetAlignedPitch(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetAlignedPitch_H(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetAlignedWidth(void);
DLL_PUBLIC MS_U16 MApi_JPEG_GetAlignedHeight(void);
DLL_PUBLIC MS_U8 MApi_JPEG_GetScaleDownFactor(void);
DLL_PUBLIC void MApi_JPEG_SetMaxDecodeResolution(MS_U16 u16Width, MS_U16 u16Height);
DLL_PUBLIC void MApi_JPEG_SetProMaxDecodeResolution(MS_U16 u16ProWidth, MS_U16 u16ProHeight);
DLL_PUBLIC void MApi_JPEG_SetMRBufferValid(JPEG_BuffLoadType u8MRBuffType);
DLL_PUBLIC void MApi_JPEG_UpdateReadInfo(MS_U32 u32DataRead, MS_BOOL bEOFflag);
DLL_PUBLIC void MApi_JPEG_ProcessEOF(JPEG_BuffLoadType u8MRBuffType);
DLL_PUBLIC void MApi_JPEG_SetErrCode(JPEG_ErrCode ErrStatus);
DLL_PUBLIC void MApi_JPEG_SetDbgLevel(MS_U8 u8DbgLevel);
DLL_PUBLIC MS_U8 MApi_JPEG_GetDbgLevel(void);
DLL_PUBLIC void MApi_JPEG_GetInfo(JPEG_Info *pJPEG_Info);
DLL_PUBLIC void MApi_JPEG_GetStatus(JPEG_Status *pJPEG_Status);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetLibVer(const MSIF_Version **ppVersion);
DLL_PUBLIC JPEG_Result MApi_JPEG_HdlVidxChk(void);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetBuffLoadType(JPEG_BuffLoadType *pBuffLoadType);
DLL_PUBLIC JPEG_Result MApi_JPEG_EnableISR(JPEG_IsrFuncCb IsrCb);
DLL_PUBLIC JPEG_Result MApi_JPEG_DisableISR(void);
DLL_PUBLIC JPEG_DecodeStatus MApi_JPEG_WaitDone(void);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFDateTime(JPEG_EXIF_DateTime *DateTime);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFOrientation(JPEG_EXIF_Orientation *eOrientation);
#if SUPPORT_EXIF_EXTRA_INFO
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFManufacturer(MS_U8 *pu8Manufacturer, MS_U8 u8size);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFModel(MS_U8 *pu8Model, MS_U8 u8size);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFFlash(MS_U16 *pu16Flash);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFISOSpeedRatings(MS_U32 *pu32ISOSpeedRatings);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFShutterSpeedValue(JPEG_RATIONAL *pShutterSpeedValue);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFApertureValue(JPEG_RATIONAL *pApertureValue);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFExposureBiasValue(JPEG_RATIONAL *pExposureBiasValue);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFFocalLength(JPEG_RATIONAL *pFocalLength);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFImageWidth(MS_U32 *pu32ImageWidth);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFImageHeight(MS_U32 *pu32ImageHeight);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFExposureTime(JPEG_RATIONAL *pExposureTime);
DLL_PUBLIC JPEG_Result MApi_JPEG_GetEXIFFNumber(JPEG_RATIONAL *pFNumber);
#endif
DLL_PUBLIC JPEG_Result MApi_JPEG_GetControl(EN_JPEG_GET_CTRL_ID eGetID, MS_U32 *param, MS_U32 u32Size);

DLL_PUBLIC void MApi_JPEG_DisableAddressConvert(void);
DLL_PUBLIC MS_PHYADDR MApi_JPEG_GetFreeMemory(MS_U32 size);
DLL_PUBLIC MS_U32 MApi_JPEG_GetDataOffset(void);
DLL_PUBLIC MS_U32 MApi_JPEG_GetSOFOffset(void);
DLL_PUBLIC JPEG_Result MApi_JPEG_SupportCMYK(MS_BOOL bEnable);
DLL_PUBLIC JPEG_Result MApi_JPEG_SupportRGB(MS_BOOL bEnable);
DLL_PUBLIC void MApi_JPEG_SetMHEG5(MS_BOOL bEnable);

#if SUPPORT_MPO_FORMAT
//For MPO
DLL_PUBLIC MS_BOOL MApi_JPEG_IsMPOFormat(void);
DLL_PUBLIC MS_BOOL MApi_JPEG_GetMPOIndex(JPEG_MPO_INDEX_INFO **ppMPOIndex);
DLL_PUBLIC MS_BOOL MApi_JPEG_GetMPOAttr(MS_U32 image_no, JPEG_MPO_ATTRIBUTE_INFO **ppMPOAttr);
DLL_PUBLIC void MApi_JPEG_DumpMPO(void);
DLL_PUBLIC MS_BOOL MApi_JPEG_SetMPOBuffer(MS_U32 read_offset, MS_U32 output_start);
DLL_PUBLIC void MApi_JPEG_SetMPOMaxDecodeResolution(MS_U16 u16Width, MS_U16 u16Height);
DLL_PUBLIC void MApi_JPEG_SetMPOProMaxDecodeResolution(MS_U16 u16ProWidth, MS_U16 u16ProHeight);
#endif
