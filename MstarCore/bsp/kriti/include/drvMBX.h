...
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_Init(MBX_CPU_ID eHKCPU, MBX_ROLE_ID eHostRole, MS_U32 u32TimeoutMillSecs);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_DeInit(MS_BOOL bForceDiscardPendingMsg);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_RegisterMSG(MBX_Class eMsgClass, MS_U16 u16MsgQueueSize);
...
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_RegisterMSGWithCallBack(MBX_Class eMsgClass, MS_U16 u16MsgQueueSize, MBX_MAIL_ARRIVE_NOTIFY_FUNC notifier);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_UnRegisterMSG(MBX_Class eMsgClass, MS_BOOL bForceDiscardMsgQueue);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_ClearMSG(MBX_Class eMsgClass);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_SendMsg(MBX_Msg *pMsg);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_GetMsgQueueStatus(MBX_Class eTargetClass, MBX_MSGQ_Status *pMsgQueueStatus);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_RecvMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, MS_U32 u32WaitMillSecs, MS_U32 u32Flag);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_CheckMsg(MBX_Class eTargetClass, MBX_Msg *pMsg, MS_U32 u32WaitMillSecs, MS_U32 u32Flag);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_SendMsgLoopback(MBX_Msg *pMsg, MBX_ROLE_ID eSrcRoleId);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_Enable(MS_BOOL bEnable);//Enable receiving regisited messages in kernel
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_RemoveLatestMsg(void);

// For Application Set/Get Information API:
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_SetInformation(MBX_ROLE_ID eTargetRole, MS_U8 *pU8Info, MS_U8 u8Size);
DLL_PUBLIC INTERFACE MBX_Result  MDrv_MBX_GetInformation(MBX_ROLE_ID eTargetRole, MS_U8 *pU8Info, MS_U8 u8Size);

// For Debug & Version Control:
DLL_PUBLIC INTERFACE void        MDrv_MBX_SetDbgLevel(MS_U8 u8DbgLevel);
DLL_PUBLIC INTERFACE MS_BOOL     MDrv_MBX_GetLibVer(const MSIF_Version **ppVersion);
DLL_PUBLIC INTERFACE MS_BOOL     MDrv_MBX_GetInfo(MBX_DrvInfo *pMbxDrvInfo);
DLL_PUBLIC INTERFACE MS_BOOL     MDrv_MBX_GetStatus(MBX_DrvStatus *pMbxDrvStatus);
DLL_PUBLIC INTERFACE void        MDrv_MBX_SetCallDrvFlag(MS_BOOL bEnable);
DLL_PUBLIC INTERFACE MS_BOOL     MDrv_MBX_GetCallDrvFlag(void);
