...
tmErrorCode_t set_video(tmHdmiTxobject_t *pDis,tmbslHdmiTxVidFmt_t reg_idx,tmHdmiTxVidReg_t *format_param)
{
   tmErrorCode_t err;
   UInt8 regVal;
   HDMI_TRACE("timing now is :%d",(int)reg_idx);
...
/*     printk("DBG auto toggle X:%d V:%d H:%d\n",toggleX,toggleV,toggleH); */
    ...
    if (EXTRA(reg_idx) && (structure3D != HDMITX_3D_FRAME_PACKING)) {
       ...
       RETIF_REG_FAIL(set_video(pDis,(tmbslHdmiTxVidFmt_t)BASE(reg_idx),(tmHdmiTxVidReg_t *)format_param_extra));
    }
    else if (EXTRA(reg_idx3D) && (structure3D == HDMITX_3D_FRAME_PACKING)) {
       /* 3d extra frame packing */
       RETIF_REG_FAIL(set_video(pDis,(tmbslHdmiTxVidFmt_t)BASE(reg_idx3D),(tmHdmiTxVidReg_t *)format_param_extra));
    }
    else {
	HDMI_TRACE("timing now fail :%d",(int)reg_idx);	 	
...
)
{
    ...
    }
    {
       ...
       RETIF_BADPARAM(reg_vid_fmt(pDis->vinFmt,(tmbslHdmiTx3DStructure_t)0,&reg_idx,0,&sync));
       if (EXTRA(reg_idx)) {
          toggle= (tmbslHdmiTxVsTgl_t)(E_MASKREG_P00_TBG_CNTRL_1_vh_tgl &                    \
                         ...
                         struct sync_desc **sync)
{
   ...
   if (sync) {
	   if PREFETCH(*idx) {
	   	   HDMI_TRACE("timing now ref_sync");
