...
#define RIU_MAP  0xBF000000//0xBF200000
...
U8 g_bIrRepeat = FALSE;//!<IR Repeat code 수신 flag
...
#elif(IR_MODE_SEL == IR_TYPE_SWDECODE_KON_MODE || IR_MODE_SEL == IR_TYPE_SWDECODE_MODE)
#define IRCHAR(a)      //putchar(a)
...
U8 g_ucIrPulseCntIndex = 0;
U8 g_ucIrRepeatTimer = 0;
static U8   g_u8TmpFlag = 0;
//static U16  g_u16IrKey=0x00;    // for store shift ir data
//static U8   g_u8IrDataBits=0; // for store bit count
//static U8   g_u8IrDataFlag;   // for store bit var
U8 g_u8IrRepeatCount;    // for repeat
U16 g_wIrPulseCounter = 0;
...
void msIR_Write3Byte( U16 u16RegIndex, U32 u32Value )
{
    ...
    *((unsigned short volatile *) RIU_MAP + u16RegIndex) = (U16)(u32Value&0xFFFF);
    ...
    //*((unsigned short volatile *) RIU_MAP + u16RegIndex + 1) = (U16)(u32Value>>16);
}
...
void HAL_IR_RC_CfgLgpsThr(void)
{
    ...
    switch(u8RCDecode)
    {
        case HAL_IR_RC_RC5:
        case HAL_IR_RC_RC5X:
            u16RefTime = IR_RC5_BIT_TIME;
            break;
        case HAL_IR_RC_RC6:
            u16RefTime = IR_RC6_BIT_TIME;
            break;
        default:
            u16RefTime = 0;
            break;
...
void HAL_IR_RC_CfgWdogCount(void)
{
    ...
    switch(u8RCDecode)
    {
        case HAL_IR_RC_RC5:
        case HAL_IR_RC_RC5X:
            u16RefTime = (IR_RC5_BIT_TIME * 14);
            break;
        case HAL_IR_RC_RC6:
            u16RefTime = (IR_RC6_BIT_TIME * 29);
            break;
        default:
            u16RefTime = 0;
            break;
...
#endif
    MS_BOOL ret;
    ret = MsOS_AttachInterrupt(E_INT_FIQ_IR, (InterruptCb)MDrv_IR_SW_Isr);
    //printf("[lht]1ret:%d\n",ret);
    ret = MsOS_EnableInterrupt(E_INT_FIQ_IR);
    //printf("[lht]2ret:%d\n",ret);
...
BOOLEAN msIR_GetIRKeyCode(U32 *pkey, U8 *pu8flag)
{
#if (IR_MODE_SEL == IR_TYPE_FULLDECODE_MODE)
    if(FALSE == g_bIsIrModeSwitchedToSWmode)
    {
    ...
    if ( g_u8TmpFlag == 1) //nec
    {
        if(g_bIrDetect)
        {
            ...
            //printf("C2:%x\n", g_ucIrCode[2]);
            //printf("C3:%x\n", g_ucIrCode[3]);
            *pkey = (g_ucIrCode[0] & 0x000000FF) << 24 | (g_ucIrCode[1] & 0x000000FF) << 16 | (IR_CODE & 0x000000FF) << 8;
            *pu8flag = FALSE;
            g_ucIrRepeatSkipCnt = IR_REPEAT_SKIP_COUNT;
            g_bIrStartReceived = FALSE;
            g_bIrDetect = FALSE;
    ...
    else if ( g_u8TmpFlag == 2 ) //konka
    {
        U8 u8Temp;
        U16 _u16Header;
        if(g_u8IrKonFlag & IR_KON_DETECT_END)
        {
            _u16Header = g_u16IrKonDecode & 0xFF00; // get headcode   xx00
            u8Temp = (U8)(g_u16IrKonDecode & 0x00FF);
            *pkey = _u16Header << 16 | u8Temp << 8;  // get key value

            if ( (g_u8IrKonFlag & IR_KON_REPEATE_TIMEOUT) || (g_u8IrKonPreKey != u8Temp) || (u8Temp==11))    //power key and sleep key
            {
                *pu8flag = 0;
                g_u8IrKonPreKey = u8Temp;
            }
            ...
            else
            {
                // repeat
                g_u8IrKonRepeatCount++;
                if (g_u8IrKonRepeatCount >= 5)//IR_FILTER_REPEAT_NUM
                {
                    return MSRET_OK;
                }
            }
...
static U8 u8ir_pressed;
typedef void (*OnIRKeyPressCB)(U8 irKey);
OnIRKeyPressCB pFunOnIREvent = NULL;

void MDrv_IR_SetOnIRKeyCB(void *pOnIRKeyCB)
{
    pFunOnIREvent = (OnIRKeyPressCB)pOnIRKeyCB;
}
...
static void MDrv_IR_SW_Isr(MHAL_SavedRegisters *pHalReg, U32 u32Data)
{
    ...
    g_u16IrKonCounter = 0;
    g_u16IrKonCounter |=  ((MDrv_ReadByte(IR_SHOT_CNT_1)) << 8);
    g_u16IrKonCounter |=  (MDrv_ReadByte(IR_SHOT_CNT_0));
...
#elif (IR_MODE_SEL == IR_TYPE_SWDECODE_MODE)
    g_wIrPulseCounter = (U16)MDrv_ReadByte(IR_SHOT_CNT_1);
    g_wIrPulseCounter <<= 8;
    ...
    if(g_bIrStartReceived == FALSE)
    {
        ...
        {
            if(g_wIrPulseCounter > 4000L && g_wIrPulseCounter < 5000L)
            {
                //printf("NL\n");
            ...
            else if(g_wIrPulseCounter > IR_KON_PWS3_HEADER_CNT_LB && g_wIrPulseCounter < IR_KON_PWS3_HEADER_CNT_UB)
            {
                g_bIrStartReceived = TRUE;
                g_u8IrKonFlag  |= IR_KON_DETECT_START;
                g_u16IrKonData = 0;
    ...
    else
    {
        ...
        {
            g_u16IrKonCounter = g_wIrPulseCounter;
            g_u8IrKonFlag |= IR_KON_COUNTER_ERROR;
            if (g_u8IrKonFlag & IR_KON_DETECT_START )
            {
                if (g_u16IrKonCounter > IR_KON_PWS3_LOGIC0_CNT_LB && g_u16IrKonCounter < IR_KON_PWS3_STOP_CNT_UB)
                {
                    g_u8IrKonFlag &= ~IR_KON_COUNTER_ERROR;
            ...
            else if (g_u16IrKonCounter > IR_KON_PWS3_STOP_CNT_LB && g_u16IrKonCounter <  IR_KON_PWS3_STOP_CNT_UB)
            {
                ...
                if(g_u8IrKonBits >= IR_KON_DATA_BITS)
                {
                    ...
                    g_u8IrKonBits = 0;
                    if (MsOS_GetSystemTime()-g_u8IrKonRepeatTimeout > IR_KON_PWS3_REPEATE_TIMEOUT)
                        g_u8IrKonFlag |= IR_KON_REPEATE_TIMEOUT;
                    else
                        g_u8IrKonFlag &= (~IR_KON_REPEATE_TIMEOUT);
                    g_u8IrKonRepeatTimeout = MsOS_GetSystemTime();
                }
                else
                {
                    g_u8IrKonFlag &= ~IR_KON_DETECT_START;
...
BOOLEAN MDrv_Power_CheckPowerOnSWIRKey(void)
{
    ...
    if(msIR_GetIRKeyCode(&u16Key, &u8Flag))
    {
        if (u16Key == IRKEY_POWER)
            return TRUE;
