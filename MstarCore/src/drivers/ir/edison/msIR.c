...
#define IR_SWDECODE_MODE_BUF_LEN 40
U16 volatile g_u16IRCounter; 
U8 volatile  g_u8IrCode;
U8 volatile  g_u8KeyData;
U8 volatile  g_u8IrBitsCnt; // IR bits counter
U16 volatile g_u16IRData[IR_SWDECODE_MODE_BUF_LEN];
BOOLEAN g_bIRStart;
...
#elif(IR_MODE_SEL == IR_TYPE_SWDECODE_TOSHIBA_MODE)
    msIR_WriteByte(IR_FIFO_CTRL, 0x2F);
...
#elif (IR_MODE_SEL == IR_TYPE_SWDECODE_TOSHIBA_MODE)
{
	...
	if(g_u8IrBitsCnt < 33)
	{
		printf("error: g_u8IrBitsCnt = %d\n",g_u8IrBitsCnt);
		goto GetIRKeyCodeLast;
	}
	if((g_u16IRData[0] > IR_TOSHIBA_OFF_CNT_UB) || ( g_u16IRData[0] < IR_TOSHIBA_OFF_CNT_LB))
	{
		printf("get Off Code error!\n");
		goto GetIRKeyCodeLast;
	}
	for(i = 1; i < 33; i++)
	{
		tmp = g_u16IRData[i];
        if((tmp >= IR_TOSHIBA_LOGIC0_CNT_LB) &&(tmp <= IR_TOSHIBA_LOGIC0_CNT_UB))
        {
            g_u8KeyData >>= 1;
            g_u8KeyData &= ~(BIT7);
        }
        else if((tmp >= IR_TOSHIBA_LOGIC1_CNT_LB) &&( tmp <= IR_TOSHIBA_LOGIC1_CNT_UB))
        {
            g_u8KeyData >>= 1;
            g_u8KeyData |= BIT7;
        }
        ...
        {
			printf("error:g_u16IRData[%d] = %d\n",i,tmp);
			goto GetIRKeyCodeLast;
        }
        switch(i)
        {
            case 0x08://one byte
                if(g_u8KeyData != IR_HEADER_CODE0)
                {
                    g_u8IrBitsCnt = 0;
					
					printf("HEADER_CODE0 error!\n");
					goto GetIRKeyCodeLast;
                }
				g_u8KeyData = 0;
                break;
            ...
            case 0x10://second byte
                if(g_u8KeyData != IR_HEADER_CODE1)
                {
                    g_u8IrBitsCnt = 0;
                    printf("HEADER_CODE1 error!\n");
					goto GetIRKeyCodeLast;
                }
				g_u8KeyData = 0;
                break;
            ...
            case 0x18://three byte
                g_u8IrCode = g_u8KeyData;
				g_u8KeyData = 0;
                break;

            case 0x20://four byte
                if((g_u8IrCode + g_u8KeyData) == 0xFF)
                {
					*pkey = g_u8IrCode;
					*pu8flag = 1;
					g_u8IrBitsCnt = 0;
...
static void MDrv_IR_SW_Isr(MHAL_SavedRegisters *pHalReg, U32 u32Data)
{
	...
	g_u16IRCounter = ((U16)(MDrv_ReadByte(IR_SHOT_CNT_1)) << 8 | (U16)(MDrv_ReadByte(IR_SHOT_CNT_0)));

	if(g_u16IRCounter > IR_TOSHIBA_OFF_CNT_LB && g_u16IRCounter < IR_TOSHIBA_OFF_CNT_UB)
	{
		g_bIRStart = TRUE;
		g_u8IrBitsCnt = 0;
	}	
	if(g_bIRStart && (g_u8IrBitsCnt < 33))
	{	
		g_u16IRData[g_u8IrBitsCnt++] = g_u16IRCounter;
	}
