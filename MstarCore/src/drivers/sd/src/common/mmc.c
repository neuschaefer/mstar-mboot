...
int scan_skew_n_set_best(struct mmc *mmc)
{
	unsigned int u32_i;
	...
	for(u32_i=0; u32_i<18; u32_i++)
	{
		...
		else
		{
			...
			{
				//printf("tuning pattern compare error\n");
			}
			else
			{
	...
	HalSdio_SlectBestSkew4(u32_Skew4Result, &stSkewer);
...
uint scr[2] __attribute__((aligned(0x80)));
uint switch_status[16] __attribute__((aligned(0x80)));
...
//printf("cmd.response[0] = %08X\n", cmd.response[0]);
//printf("cmd.response[1] = %08X\n", cmd.response[1]);
//printf("cmd.response[2] = %08X\n", cmd.response[2]);
//printf("cmd.response[3] = %08X\n", cmd.response[3]);

//printf("a. mmc->read_bl_len = %d\n", mmc->read_bl_len);

	mmc->read_bl_len = 1 << ((cmd.response[1] >> 16) & 0xf);

//printf("b. mmc->read_bl_len = %d\n", mmc->read_bl_len);
	...
	if (IS_SD(mmc))
	{
		//printf("SD card, chagne freq\n");
		err = sd_change_freq(mmc); // CMD6 switch high speed in this function
	}
	else
	{
		//printf("MMC card, chagne freq\n");
	...
	if (IS_SD(mmc))
	{
		...
		if( (mmc->volt_sw_18v) && (mmc->card_caps & SD30_MODE_SDR104) )
		{
			...
			HalFcie_clock_setting(0x90); // eMMC_PLL_CLK_200M

			HalFcie_SwitchPad(6); // SDIO_MODE_32BITS_MACRO_HS200

			scan_skew_n_set_best(mmc);
		...
		else if( (mmc->volt_sw_18v) && (mmc->card_caps & SD30_MODE_SDR50) )
		{
			...
			HalFcie_clock_setting(0x8C); // eMMC_PLL_CLK_100M
	...
	if(mmc->capacity > 1<<30) // GB
	{
		printf("Find SD %llu.%llu GB [%llXh]\n", mmc->capacity>>30, (mmc->capacity&0x3FFFFFFFULL)>>20, mmc->capacity);
	}
	else
	{
		printf("Find SD %llu MB [%llXh]\n", mmc->capacity>>20, mmc->capacity);
	}
...
int mmc_init(struct mmc *mmc)
{
	...
	HalFcie_SetVDD(0); // power off
	HalFcie_SetIOVoltage(0); 	// turn off IO voltage LDO
	udelay(1000000);udelay(1000000); // delay 500 ms


	HalFcie_SetVDD(1); // power on
	HalFcie_SetIOVoltage(1); // 3.3V IO voltage
	...
	if(mmc->volt_sw_18v)
	{
		sd_voltage_switch(mmc);

		//printf("volt_sw_18v switch to 1.8\n");

		HalFcie_SetIOVoltage(2); // 1.8V
		udelay(5000); // 5ms
		if(HalFcie_SendClocknWaitD0High())
		{
