...
static void announce_and_cleanup(void)
{
    ...
    PiuTick |= (*(volatile unsigned int *)(0x1f006094)) << 16;
    unsigned int PiuTime = PiuTick / 12000;
	*(volatile unsigned int *)(0x1f007b74)=PiuTime;
    printf("[Mstar Boot up time check] Mboot Total Time:[%u]\n", PiuTime);
	printf("[Mstar Boot up time check] System Boot up time:[%u]\n", PiuTime);
    if(Mboot_time<PiuTime)
    {
        printf(" \n");
        printf("This is bug!!! Mboot boot up time surpass maximum time:[%u] \n", Mboot_time);
    }
