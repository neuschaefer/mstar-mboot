				...
				int cancel)
{
    ...
    struct ubi_wl_prot_entry *pe=NULL;//uninitialized_var(pe);
...
int ubi_wl_init_scan(struct ubi_device *ubi, struct ubi_scan_info *si)
{
	...
	if (!ubi->lookuptbl)
		return err;
	memset(ubi->lookuptbl, 0, ubi->peb_count * sizeof(void *));
	...
	/* mboot has no thread and the eba tbl is initialized after wl init, so do not ensure wear */
	//err = ensure_wear_leveling(ubi);
	//if (err)
		//goto out_free;
