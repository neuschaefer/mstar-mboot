...
retry:
	...
	if (err) {
		...
		if (read != len && retries++ < UBI_IO_RETRIES) {
			printf("error %d while reading %d bytes from PEB %d:%d, "
...
int ubi_io_sync_erase(struct ubi_device *ubi, int pnum, int torture)
{
	...
	if(!ubi->tsm->disable_flag)
	{
		...
		check_set_dirty_group(ubi, pnum);
		eninit_msg("[%s]\t pnum %d, ec is %lld", __FUNCTION__, pnum, be64_to_cpu(ubi->trn[pnum].etr.ec));
	}
#endif
	err = do_sync_erase(ubi, pnum);
	if (err)
	...
	if (magic != UBI_EC_HDR_MAGIC) {
		...
		if (read_err != -EBADMSG &&
			...
			return UBI_IO_PEB_EMPTY;
              ...
              // For the case of uncorrectable ecc error
              if (-EBADMSG == read_err)
              {
                  return read_err;
              }
			    ...
			    const struct ubi_vid_hdr *vid_hdr)
{
    ...
    #ifndef CONFIG_MTD_UBI_BACKUP_LSB
		dbg_err("bad compat");
		goto bad;
    #else
			 ...
			 struct ubi_vid_hdr *vid_hdr)
{
	...
	if(!ubi->tsm->disable_flag)
	{
		...
		check_set_dirty_group(ubi, pnum);
		eninit_msg("[%s]\t pnum %d, lnum is %d", __FUNCTION__, pnum, be32_to_cpu(ubi->trn[pnum].etr.lnum));
	}
#endif
        ...
        int pnum, int offset)
{
    ...
    err = ubi->mtd->read_oob(ubi->mtd, addr, &ops);
    //ubi_io_read(ubi, databuf, pnum, offset, ubi->min_io_size);
    //err = ubi->mtd->read_ecc(ubi->mtd, addr, ubi->min_io_size, &read,
    //          databuf, oobbuf, NULL);
    if (err) {
        if (err == -EUCLEAN) {
            /*
        ...
        int pnum, int offset)
{
    ...
    err = ubi->mtd->write_oob(ubi->mtd, addr, &ops);
    if (err) {
