...
int hdeleteall_r(struct hsearch_data *env_htab)
{
    if(env_htab==NULL)
    {
        return -1;
    ...
    int i=0;
    for(i=0;i<env_htab->size;i++)
    {
        if(env_htab->table[i].used>0)
        {
            free(env_htab->table[i].entry.data);
            free(env_htab->table[i].entry.key);
            --env_htab->filled;
            env_htab->table[i].used=-1;
        }
    }
    return 0;
}
